...
def test_replacements(gl_major_version, command):
  ...
  if not shared.find_command_file(gl_major_version, command_test) == False:
    return command_test
  ...
  if command_test != command:
    test = test_extensions(gl_major_version, command_test)
    if len(test):
      return test
  ...
  if command_test != command:
    return test_extensions(gl_major_version, command_test)
  ...
  if feature.attrib['api'] == 'gl':
    ...
    for command in current_command_list:
      ...
      if os.path.exists(gldir):
        command_file = shared.find_command_file(gl_major_version, command)
        if command_file == False:
          command_docs = test_replacements(gl_major_version, command)

          command_file = shared.find_command_file(gl_major_version, command_docs)
        if command_file == False:
          print "No command docs file found for " + command + " (" + gl_major_version + ")"
          assert(os.path.exists(gldir + '/' + command_docs + ".xhtml"))
