...
parser = argparse.ArgumentParser(description="Compile GLSL documentation, generate a static webpage.")
...
# End of html generation stuff
  
  
index_commands_version = glsl.commands_version_flat.keys()
...
for command in index_commands_version:
  ...
  if command in glsl.function_aliases and command != glsl.function_aliases[command] and glsl.function_aliases[command] in index_commands_version:
    continue
...
for version in glsl.version_commands:
  ...
  for command in glsl.version_commands[version]:
    ...
    if command != glsl.version_commands[version][command]:
      search_function_aliases[version[:2]][command] = glsl.version_commands[version][command]
...
unhandled_commands = glsl.commands_version_flat.keys()
...
version_numbers = glsl.version_commands.keys()
...
for version in major_versions:
  ...
  for command in glsl.commands_version_flat:
    ...
    for category in glsl.command_categories:
      api_commands += spew_category(category, glsl.command_categories[category], command)
    ...
    for major_version in command_major_versions:
      ...
      el = ""
      if major_version[:2] == 'el':
        el = "ES "
    
      command_versions += "<a " + link_class + " href='../" + major_version + "/" + command + "'>GLSL " + el + major_version[2] + "</a><br />"
